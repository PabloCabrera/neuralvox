...
char translate_phoneme (char *phonemes);
...
struct pron_timing *get_pron_timings (FILE *pho_file) {
	...
	while (getline ((&line_buffer), &line_length, pho_file) > -1) {
		...
		char line_phoneme = translate_phoneme (phoneme_chars);
		if (line_phoneme != '\0') {
...
char translate_phoneme (char *phonemes) {
	if ((phonemes[0] == 0) || (phonemes[0] == '_')) {
		return '\0';
	} else if (strlen (phonemes) > 1) {
		if (strcmp (phonemes, "l^") == 0) {
			return 'Y';
		} else if (strcmp (phonemes, "tS") == 0) {
			return 'H';
		} else {
			fprintf (stderr, "Warning: no se reconoce el fonema %s", phonemes);
			return '\0';
		}
	} else {
		return phonemes[0];
